...
//       3 was used in Spring 2016 run and will be used in Fall 2016 run.
//
//////////////////////////////////////////////////////////////////////////
//
// Gmp_Track_Eloss
...
void Gmp_Track_Eloss::CalcEloss( THaTrackInfo* trkifo )
{
  ...
  if( fElectronMode ) {
    ...
    if(fZmed < 1.01 && fAmed < 1.01){
      ...
      Double_t T_Al = 2.03E-4 ; // Aluminum Wall Thickness (m) 
      Double_t R = 0.0334 ; //1.315 inch
      Double_t R1 = R ; // Inner Radius for End-Cap
      Double_t R2 = R + 1.47E-4 ; // Outer Radius for End-Cap
	...
	l_Al1 = TMath::Sqrt( (R2*R2) - (TMath::Abs(Z_rel*Z_rel)*sin2) ) - TMath::Sqrt( (R1*R1) - (TMath::Abs(Z_rel*Z_rel)*sin2) ) ;
      ...
      l_Air = fAirLength ; // 13.95" (RHRS), 10.62" (LHRS)
