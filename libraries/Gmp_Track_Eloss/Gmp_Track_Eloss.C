  ...
  fTrackModule(NULL)
{
  ...
  T_Al = R = R1 = R2 = 0; // These values will be set by the fConfig flag
  fAirLength = 0; // Set by the fConfig flag, different for left and right HRS
...
THaAnalysisObject::EStatus Gmp_Track_Eloss::Init( const TDatime& run_time )
{
  ...
  TString spectr = spectro->GetName();
  ...
  switch (fConfig.Atoi()) {
      case 1:
          ...
          R1 = R;
          R2 = R+0.132E-3;
          if (spectr.Contains("L")) { // LHRS
              fAirLength = 0.2697;    
          } else { // set to RHRS by default
              fAirLength = 0.3543;
          }
#ifdef WITH_DEBUG
          if (fDebug>0)
              Info( Here(here), "Using the loop 2 configuration of LH2 target " 
                      "in Spring 2015 Gmp run for spectrometer %s.", spectr.Data());
#endif
          break;
      case 2:
          T_Al = 0.203E-3;
          ...
          if (fDebug>0)
              Info( Here(here), "Using the loop 3 configuration of LH2 target "
                      "in Spring 2016 Gmp run for spectrometer %s.", spectr.Data());
#endif
          break;
      default:
          T_Al = 0.18E-3;
          ...
          if (fDebug>0)
              Info( Here(here), "Using the loop 2 configuration of LH2 target "
                      "in Fall 2016 Gmp run for spectrometer %s.", spectr.Data());
#endif
  }
