...
// Barak Schmookler, August 2016: Pick best hit in multi-hit TDCs            // 
///////////////////////////////////////////////////////////////////////////////
...
Gmp_Xscin::Gmp_Xscin( const char* name,
...
//_____________________________________________________________________________
THaAnalysisObject::EStatus Gmp_Xscin::Init( const TDatime& date )
{
  ...
  const DataDest tmp[NDEST] = {
    { &fRTNhit, &fRANhit, fRT, fRT_c, fRA, fRA_p, fRA_c, fROff, fRPed, fRGain , fRPeak},
    { &fLTNhit, &fLANhit, fLT, fLT_c, fLA, fLA_p, fLA_c, fLOff, fLPed, fLGain , fLPeak}
  };
...
//_____________________________________________________________________________
Int_t Gmp_Xscin::ReadDatabase( const TDatime& date )
{
  ...
  if( !fIsInit ) {
    ...
    fLPeak = new Double_t[ fNelem ];
    fRPeak = new Double_t[ fNelem ];
    ...
    fXt     = new Double_t[ fNelem ];
    fXa     = new Double_t[ fNelem ];
  ...
  DBRequest list[] = {
    ...
    { "TDC_peakL", fLPeak, kDouble, static_cast<UInt_t>(fNelem) },
    { "TDC_peakR", fRPeak, kDouble, static_cast<UInt_t>(fNelem) },
    { "TDC_res",   &fTdc2T },
    { "TransSpd",  &fCn },
    { "AdcMIP",    &fAdcMIP },
  ...
  while ( ReadComment( fi, buf, LEN ) ) {}
  for (i=0;i<fNelem;i++) 
    fscanf (fi,"%lf",fLPeak+i);                  // Left Pads TDC Peaks
  fgets ( buf, LEN, fi );
  while ( ReadComment( fi, buf, LEN ) ) {}
  for (i=0;i<fNelem;i++) 
    fscanf (fi,"%lf",fRPeak+i);                  // Right Pads TDC Peaks
  fgets ( buf, LEN, fi );
...
//_____________________________________________________________________________
Int_t Gmp_Xscin::DefineVariables( EMode mode )
{
...
//_____________________________________________________________________________
Int_t Gmp_Xscin::Decode( const THaEvData& evdata )
{
  ...
  for( Int_t i = 0; i < fDetMap->GetSize(); i++ ) {
	...
	// For TDCs, get number of hits for this channel and loop through hits
	Int_t nHits = evdata.GetNumHits(d->crate, d->slot, chan);
	peak_data = 0;
	if(nHits>0) peak_data = -5000; //'Large' Negative Number	
	
	for (Int_t hit = 0; hit < nHits; hit++) {
	  
	  // Now get the TDC data for this hit
	  data = evdata.GetData(d->crate, d->slot, chan, hit);
	  if(k<fNelem){ //Right Side
	    Int_t elem = k % fNelem;
	    if( fabs(data-fRPeak[elem])<fabs(peak_data-fRPeak[elem]) ) peak_data = data;
	  }
	  else{ //Left Side
	    Int_t elem = k % fNelem;
	    if( fabs(data-fLPeak[elem])<fabs(peak_data-fLPeak[elem]) ) peak_data = data;
	  }
	}
      }
...
//        cout << "adc,j,k = " <<adc<<","<<j<< ","<<k<< endl;
	...
	dest->tdc[k]   = static_cast<Double_t>( peak_data );
	dest->tdc_c[k] = (peak_data - dest->offset[k])*fTdc2T;
	(*dest->nthit)++;
      }
    }
  }
...
//_____________________________________________________________________________
Int_t Gmp_Xscin::ApplyCorrections( void )
{
...
//_____________________________________________________________________________
Double_t Gmp_Xscin::TimeWalkCorrection(const Int_t& paddle,
					     const ESide side)
{
...
//_____________________________________________________________________________
Int_t Gmp_Xscin::CoarseProcess( TClonesArray& /* tracks */ )
{
...
//_____________________________________________________________________________
Int_t Gmp_Xscin::FineProcess( TClonesArray& tracks )
{
  ...
  for ( int i=0; i<n_track; i++ ) {
    ...
    if ( ! CalcTrackIntercept(theTrack, pathl, xc, yc) ) { // failed to hit
      new ( (*fTrackProj)[i] )
	THaTrackProj(xc,yc,pathl,dy,pad,this);
      continue;
    }
